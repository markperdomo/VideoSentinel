...
const details = () => ({
  id: 'Tdarr_Plugin_MC93_Migz1FFMPEG_CPU_modified',
  Stage: 'Pre-processing',
  Name: 'Migz Transcode Using CPU & FFMPEG (Modified for Apple)',
  Type: 'Video',
  Operation: 'Transcode',
  Description: `Files not in H265 will be transcoded into H265 using CPU with ffmpeg. Settings are dependant on file bitrate Working by the logic that H265 can support the same ammount of data at half the bitrate of H264. This plugin will skip any files that are in the VP9 or AV1 codec.`,
  Version: '1.10',
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  if (file.ffProbeData && file.ffProbeData.format && parseFloat(file.ffProbeData.format.duration) > 0) {
    duration = parseFloat(file.ffProbeData.format.duration) * 0.0166667;
  } else if (file.meta && typeof file.meta.Duration !== 'undefined') {
    duration = file.meta.Duration * 0.0166667;
  } else if (file.ffProbeData && file.ffProbeData.streams && file.ffProbeData.streams[0]) {
    duration = file.ffProbeData.streams[0].duration * 0.0166667;
  } else {
    // Fallback if no duration found to avoid division by zero
    duration = 1;
  }
  ...
  // eslint-disable-next-line no-bitwise
  const targetBitrate = ~~(file.file_size / (duration * 0.0075) / 1.8);
  ...
  for (let i = 0; i < file.ffProbeData.streams.length; i++) {
    ...
    if (codec_type === 'video') {
      ...
      // SKIP CHECK (HEVC, VP9, AV1)
      if (
        (file.ffProbeData.streams[i].codec_name === 'hevc' || file.ffProbeData.streams[i].codec_name === 'vp9' || file.ffProbeData.streams[i].codec_name === 'av1')
      ...
      if (
        ...
        // --- FORCE APPLE COMPATIBILITY (REMUX) ---
        // If outputting to MP4, ensure hvc1 tag is present
        if (inputs.container.toLowerCase() === 'mp4' || response.container === '.mp4') {
  ...
  // --- FORCE APPLE COMPATIBILITY (TRANSCODE) ---
  // If we reached here, we are transcdoing.
  if (inputs.container.toLowerCase() === 'mp4' || response.container === '.mp4') {
      extraArguments += '-tag:v hvc1 ';
  }
